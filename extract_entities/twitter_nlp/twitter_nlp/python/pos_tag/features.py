...
class POSFeatureExtractor:
    ...
    def get_features(self, token):
        ...
        for dictname, dictionary in self.occurences.items():
            if ltoken in dictionary.token_pos_set:
    ...
    def _check_bigrams(self, word1, word2, use_first):
        ...
        new_features = []
        for dict_name, d in self.bigram_dictionaries.items():
            if bigram != ":_(" and bigram in d.token_pos_set:
                #print bigram
                tag1, tag2 = list(d.token_pos_set[bigram].items())[0][0].split('_')
