...
cluster_prob = {}
for cid, clust_words in clusters_known.items():
    temp_counts = {}
    if len(clust_words) < KNOWN_LIM:
        continue
    for pos, pws in pos_words.items():
        for w in clust_words:
            if w in pws:
                temp_counts.setdefault(pos, 0)
                temp_counts[pos] += 1
    d_given_c = {}
    for pos, c in temp_counts.items():
